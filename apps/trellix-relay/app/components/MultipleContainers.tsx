...
export function MultipleContainers({
  ...
  const [items, setItems] = useState<Items>(
    () =>
      initialItems ?? {
        A: range(itemCount).map((index) => `A${index + 1}`),
        B: range(itemCount).map((index) => `B${index + 1}`),
        C: range(itemCount).map((index) => `C${index + 1}`),
        D: range(itemCount).map((index) => `D${index + 1}`),
      },
  );
  ...
  const getIndex = (id: UniqueIdentifier) => {
    ...
    const index = exists(items[container]).indexOf(id);
  ...
  function renderContainerDragOverlay(containerId: UniqueIdentifier) {
    return (
      <Container
        ...
        unstyled={false}
      >
        {exists(items[containerId]).map((item, index) => (
          <Item
            key={item}
            value={item}
            handle={handle}
